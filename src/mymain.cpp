...
wxPanel *MyFrame::BuildControlsPanel(wxWindow *parent)
{
    ...
    // wxButton *button = new wxButton(this, wxID_ANY, "Capture Screenshot", wxPoint(10, 10));

    // Bind the event for button click
    // button->Bind(wxEVT_BUTTON, &DrawingCanvas::OnCaptureScreenshot, this);
    // button->Bind(wxEVT_BUTTON, &DrawingCanvas::OnPaint, this);
...
canvas->OnCaptureScreenshot(dummyEvent); // Add this method in your DrawingCanvas class
} });

    mainSizer->Add(button, 0, wxALL, FromDIP(5));

    //  button->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &DrawingCanvas::OnCaptureScreenshot, this);
